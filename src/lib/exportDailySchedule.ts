...
export async function exportDailyScheduleExcel(
  ...
  // Duty teachers section
  if (dutyTeachers.length > 0 && dutyTeachers.some(dt => dt.name)) {
    ws.addRow([]); // spacing
...
export async function exportDailyScheduleDocx(
  ...
  function hdrCell(text: string, width?: number): DocxTC {
    return new DocxTC({
      ...
      children: [new Paragraph({
        ...
        children: [new TextRun({ text, font: FONT_NAME, bold: true, size: 22, color: "FFFFFF" })],
      })],
    });
  }
  ...
  function dCell(lines: string[], empty = false): DocxTC {
    return new DocxTC({
      ...
      children: lines.length > 0 ? lines.map(l => new Paragraph({
        ...
        children: [new TextRun({ text: l, font: FONT_NAME, size: 20 })],
  ...
  const children: any[] = [
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 100 },
      children: [new TextRun({ text: schoolName, font: FONT_NAME, bold: true, size: 32, color: HEADER_BG })],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 100 },
      children: [new TextRun({ text: `الجدول اليومي - يوم ${dayName}`, font: FONT_NAME, bold: true, size: 28, color: ACCENT_BG })],
    }),
  ];

  // Absent teachers line
  if (absentTeacherNames.length > 0) {
    children.push(new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 200 },
      children: [
        new TextRun({ text: "المعلمون الغائبون: ", font: FONT_NAME, bold: true, size: 22, color: ABSENT_BG }),
        new TextRun({ text: absentTeacherNames.join(" ، "), font: FONT_NAME, size: 22, color: ABSENT_BG }),
      ],
    }));
  }
  ...
  if (dutyTeachers.length > 0 && dutyTeachers.some(dt => dt.name)) {
    children.push(new Paragraph({
      ...
      children: [new TextRun({ text: "المناوبون", font: FONT_NAME, bold: true, size: 24, color: HEADER_BG })],
    }));
    ...
    const dutyHeaderRow = new DocxTR({
      ...
      children: [
        ...
        new DocxTC({
          ...
          children: [new Paragraph({
            alignment: AlignmentType.CENTER,
            children: [new TextRun({ text: "اسم المناوب", font: FONT_NAME, bold: true, size: 22, color: HEADER_BG })],
          })],
        }),
        new DocxTC({
          shading: { type: ShadingType.SOLID, color: ACCENT_BG },
          ...
          children: [new Paragraph({
            alignment: AlignmentType.CENTER,
            children: [new TextRun({ text: "المكان", font: FONT_NAME, bold: true, size: 22, color: HEADER_BG })],
          })],
        }),
      ],
    });
