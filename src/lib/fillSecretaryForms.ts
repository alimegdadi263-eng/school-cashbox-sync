  ...
  const doc = new Document({
    sections: [{
      ...
      children: [
        ...
        gap(20),
        p([t("الجزء الأول: (يعبأ من قبل مسؤول شؤون الموظفين)", { bold: true, size: M, underline: true })], AlignmentType.LEFT),
        gap(10),
        part1,
        p([t(`العقوبات التأديبية السابقة المتخذة بحق الموظف: ${data.previousPenalties || dots(50)}`, { size: S })], AlignmentType.LEFT),
        gap(10),
        p([t("الجزء الثاني: (يعبأ من قبل الرئيس المباشر للموظف)", { bold: true, size: M, underline: true })], AlignmentType.LEFT),
        p([t(`موضوع الاستفسار: ${data.subject || ""}`, { size: M })], AlignmentType.LEFT),
        p([t(data.details || "", { size: S })], AlignmentType.LEFT),
        gap(80),
        p([t("التوقيع:", { bold: true, size: M })], AlignmentType.LEFT),
        gap(10),
        p([t("الجزء الثالث: (يعبأ من قبل الموظف المستجوب)", { bold: true, size: M, underline: true })], AlignmentType.LEFT),
        p([t("الإجابة:", { size: M })], AlignmentType.LEFT),
        gap(120),
        p([t("التوقيع:", { bold: true, size: M })], AlignmentType.LEFT),
        gap(10),
        p([t("الجزء الرابع: القرار متخذ وفقاً للصلاحيات المنصوص عليها في المادة (68/أ) من إدارة الموارد البشرية رقم (33) لسنة 2024", { size: S })], AlignmentType.LEFT),
        p([t(`تنسيب / قرار مدير المدرسة: ${dots(50)}`, { size: S })], AlignmentType.LEFT),
        p([t("التوقيع:", { size: S })], AlignmentType.LEFT),
        p([t(`تنسيب / قرار مدير التربية والتعليم: ${dots(50)}`, { size: S })], AlignmentType.LEFT),
        p([t("التوقيع:                              التاريخ:    /    /", { size: S })], AlignmentType.LEFT),
        p([t(`تنسيب / قرار الأمين العام: ${dots(50)}`, { size: S })], AlignmentType.LEFT),
        p([t("التوقيع:                              التاريخ:    /    /", { size: S })], AlignmentType.LEFT),
        p([t(`تنسيب /قرار الرئيس: ${dots(50)}`, { size: S })], AlignmentType.LEFT),
        p([t("التوقيع:                              التاريخ:    /    /", { size: S })], AlignmentType.LEFT),
      ],
    }],
  });
...
export async function fillCasualLeaveForm(data: CasualLeaveData) {
  ...
  const infoTable = new Table({
    rows: [
      ...
      new TableRow({ children: [
        c(`المسمى الوظيفي: ${data.jobTitle || ""}`, { width: 33, align: AlignmentType.LEFT }),
        c(`الرقم الوزاري: ${data.employeeNumber || ""}`, { width: 34, align: AlignmentType.LEFT }),
        c(`الاسم: ${data.employeeName}`, { width: 33, align: AlignmentType.LEFT }),
      ]}),
      new TableRow({ children: [
        c(`الشعبة: ${data.department || data.school}`, { width: 33, align: AlignmentType.LEFT }),
        c(`القسم: ${data.section || ""}`, { width: 34, align: AlignmentType.LEFT }),
        c(`الإدارة:`, { width: 33, align: AlignmentType.LEFT }),
      ]}),
      new TableRow({ children: [
        c("", { width: 33 }),
        c(`المديرية: ${data.directorate || "لواءي الطيبة والوسطية"}`, { width: 34, align: AlignmentType.LEFT }),
        c("", { width: 33 }),
      ]}),
    ],
    width: { size: 100, type: WidthType.PERCENTAGE },
  ...
  const doc = new Document({
    sections: [{
      ...
      children: [
        ...
        gap(10),
        leaveHeader,
        p([t("سبب الإجازة:", { bold: true, size: M })], AlignmentType.LEFT),
        p([t("وفاة أحد الأقارب:  ☐  من الدرجة الأولى    من الدرجة الثانية    من الدرجة الثالثة    زوجة/ زوج", { size: S })], AlignmentType.LEFT),
        p([t("☐ أسباب أخرى ( خاص بمن تنطبق عليهم أحكام المادة (35-ج) من نظام إدارة الموارد البشرية في القطاع العام):", { size: S })], AlignmentType.LEFT),
        p([t(data.otherReasons || data.leaveReason || dots(60), { size: S })], AlignmentType.LEFT),
        p([t(dots(80), { size: S })], AlignmentType.LEFT),
        p([t(`عدد الأيام المستحقة بموجب أحكام المواد (53) و(54) من نظام إدارة الموارد البشرية في القطاع العام: ( ${data.daysEntitled || "  "} ) يوم`, { size: S })], AlignmentType.LEFT),
        p([t(`مجموع الإجازات العرضية التي تم منحها للموظف خلال العام: ( ${data.totalLeavesThisYear || "  "} ) يوم`, { size: S })], AlignmentType.LEFT),
        p([t(`تاريخ ابتداء الإجازة: ${data.startDate || "  /  /  "}                    تاريخ انتهاء الإجازة: ${data.endDate || "  /  /  "}`, { size: S })], AlignmentType.LEFT),
        p([t(`ملاحظات: ${data.notes || dots(60)}`, { size: S })], AlignmentType.LEFT),
        p([t(dots(80), { size: S })], AlignmentType.LEFT),
        gap(20),
        p([t("الأسم:                    المسمى الوظيفي:                    التوقيع:", { size: M })], AlignmentType.LEFT),
        gap(10),
        new Table({
        ...
        p([t(dots(80), { size: S })], AlignmentType.LEFT),
        gap(20),
        p([t("التاريخ:    /    /                                                    التوقيع", { size: M })], AlignmentType.LEFT),
      ],
    }],
  });
  ...
  function buildPage(pageItems: InventoryCustodyItem[], pageNum: number, totalPages: number) {
    ...
    const dataRows = pageItems.map(item => {
      ...
      return new TableRow({ children: [
        ...
        c(item.itemName, { align: AlignmentType.LEFT }),
    ...
    return [
      ...
      table,
      gap(20),
      p([t("أقر أنا المسؤول عن العهدة بأن الجرد تم بحضوري ومعرفتي وأوافق على صحة القوائم ونتائجها من حيث النقص أو الزيادة ولا توجد أية مستودعات أخرى للوازم بالمدرسة غير التي جرى عليها الجرد وعليه أوقع على هذا المحضر", { size: 16 })], AlignmentType.LEFT),
      gap(10),
      ...
      new Table({
        rows: [
          ...
          new TableRow({ children: [
            c(`الاسم: ${data.custodian || ""}`, { align: AlignmentType.LEFT }),
            c(`الاسم: ${data.directorName}`, { align: AlignmentType.LEFT }),
            c(`الاسم: ${data.committeeMember1 || ""}`, { align: AlignmentType.LEFT }),
            c(`الاسم: ${data.committeeMember2 || ""}`, { align: AlignmentType.LEFT }),
          ]}),
          new TableRow({ children: [
            c("التوقيع:", { align: AlignmentType.LEFT }),
            c("التوقيع:", { align: AlignmentType.LEFT }),
            c("التوقيع:", { align: AlignmentType.LEFT }),
            c("التوقيع:", { align: AlignmentType.LEFT }),
          ]}),
          new TableRow({ children: [
            c(`التاريخ: ${data.date}`, { align: AlignmentType.LEFT }),
            c(`التاريخ: ${data.date}`, { align: AlignmentType.LEFT }),
            c(`التاريخ: ${data.date}`, { align: AlignmentType.LEFT }),
            c(`التاريخ: ${data.date}`, { align: AlignmentType.LEFT }),
          ]}),
  ...
  function buildPage(pageItems: DisposalDocxItem[], pageNum: number, totalPages: number) {
    ...
    const dataRows = pageItems.map(item => {
      ...
      return new TableRow({ children: [
        ...
        c(item.grade || "", { size: 14, align: AlignmentType.LEFT }),
        c(item.itemName || "", { size: 14, align: AlignmentType.LEFT }),
    ...
    return [
      ...
      p([t(`نشهد نحن أعضاء لجنة التبرعات في مدرسة (${data.school}) بأننا عاينا الكتب المذكورة أعلاه فوجدناها ملغاة وغير صالحة للاستعمال، ونوصي باتلافها حسب الطبعات المعتمدة.`, { size: 16 })], AlignmentType.LEFT),
      gap(10),
      ...
      new Table({
        rows: [
          ...
          new TableRow({ children: [
            c(`الاسم: ${data.directorName}`, { align: AlignmentType.LEFT, size: 14 }),
            c(`الاسم: ${data.committeeMember1 || ""}`, { align: AlignmentType.LEFT, size: 14 }),
            c(`الاسم: ${data.committeeMember2 || ""}`, { align: AlignmentType.LEFT, size: 14 }),
            c(`الاسم: ${data.committeeMember3 || ""}`, { align: AlignmentType.LEFT, size: 14 }),
          ]}),
          new TableRow({ children: [
            c("التوقيع:", { align: AlignmentType.LEFT, size: 14 }),
            c("التوقيع:", { align: AlignmentType.LEFT, size: 14 }),
            c("التوقيع:", { align: AlignmentType.LEFT, size: 14 }),
            c("التوقيع:", { align: AlignmentType.LEFT, size: 14 }),
          ]}),
          new TableRow({ children: [
            c("التاريخ:", { align: AlignmentType.LEFT, size: 14 }),
            c("التاريخ:", { align: AlignmentType.LEFT, size: 14 }),
            c("التاريخ:", { align: AlignmentType.LEFT, size: 14 }),
            c("التاريخ:", { align: AlignmentType.LEFT, size: 14 }),
          ]}),
      ...
      gap(5),
      p([t("ملاحظة: تنظم قائمة لكل سجل على حدة على غرار هذا النموذج وعلى أربع نسخ", { size: 14 })], AlignmentType.LEFT),
