...
const sections: DocSection[] = [
  ...
  },
  {
    ...
    color: "text-lime-500",
    items: [
      { file: "src/components/studentAbsence/AjyalIntegration.tsx", description: "واجهة ربط أجيال: تدعم طريقتين لتسجيل الدخول — (1) اسم مستخدم وكلمة مرور مع تعبئة تلقائية + OTP، (2) الدخول عبر تطبيق سند (SANAD) بدون إدخال بيانات. تتيح تبويبات لتعبئة الغياب واستيراد بيانات الطلاب." },
      { file: "electron/main.cjs (Ajyal handlers)", description: "IPC Handlers في Electron: ajyal-open-embedded يفتح BrowserView على العنوان الرسمي لمنصة أجيال مع شريط تحكم محقون (Toolbar). يدعم وضعين: credentials (تعبئة تلقائية) و sanad (دخول يدوي). يتضمن نظام اكتشاف الصفوف تلقائياً (discoverAndShowGradePanel) مع واجهة اختيار صفوف (checkboxes) تتيح استيراد/تعبئة (الكل) أو صفوف محددة. يدعم إلغاء العملية وإعادة التنفيذ بدون إعادة تشغيل البرنامج." },
      { file: "electron/preload.cjs (ajyal API)", description: "واجهة IPC آمنة: يوفر electronAPI.ajyal مع الدوال openWindow(username, password, loginMethod)، و checkLogin، و submitAbsence، و importStudents، و closeWindow عبر contextBridge. يمنع الوصول المباشر لـ Node.js." },
      { file: "آلية اكتشاف الصفوف", description: "عند الضغط على (استيراد الطلاب) أو (تعبئة الغياب)، يتنقل النظام تلقائياً لصفحة الطلاب ويقرأ خيارات قائمة الصفوف المنسدلة (getSelectOptionsJS). ثم يحقن لوحة اختيار بصرية (grade-selection-panel) مع خيار (الكل) وعلامات ✅ لكل صف. المستخدم يحدد الصفوف ويضغط (تنفيذ) لبدء العملية على الصفوف المختارة فقط." },
      { file: "آلية التعبئة التلقائية", description: "عند تنفيذ تعبئة الغياب، يتم تجميع سجلات الغياب حسب الصف (byClass)، ثم فلترتها بالصفوف المختارة (filteredClassKeys). لكل صف: يتم ضبط التاريخ، اختيار الصف والشعبة من القوائم المنسدلة بمطابقة جزئية، ثم البحث عن كل طالب في الجدول ووضع علامة غياب مع التحقق من تطابق الاسم والصف." },
      { file: "استيراد الطلاب من أجيال", description: "يقرأ DOM صفحة قائمة الطلاب ويستخرج: اسم الطالب، الصف/الشعبة، ورقم ولي الأمر. يحدد عمود الاسم من ترويسة الجدول (header row). يدعم استيراد صفوف محددة عبر selectedGrades filter. يتم دمج النتائج مع القائمة الحالية مع تجنب التكرار." },
      ...
      { file: "استمرارية الجلسة", description: "يتم إخفاء/إظهار BrowserView بدلاً من إغلاقه عند الضغط على (رجوع)، مما يحافظ على تسجيل الدخول. شريط التحكم يُعاد تفعيله تلقائياً عند العودة لأجيال. الأزرار تُقفل أثناء التنفيذ وتُفتح تلقائياً بعد انتهاء العملية أو حدوث خطأ (finally block)." },
    ],
  },
  {
