...
// Split amount into dinars and fils
...
// Get total debit and credit for a transaction
      ...
      <head>
        ...
        <style>
          ...
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .description-col { text-align: right; min-width: 200px; }
          .total-row { background: #e8e8e8; font-weight: bold; }
          .accounts { margin: 10px 0; font-size: 14px; line-height: 1.8; }
          ...
          .sig-label { font-weight: bold; font-size: 13px; margin-bottom: 30px; }
          .sig-line { border-top: 1px solid #000; margin-top: 30px; padding-top: 5px; font-size: 12px; }
    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div ref={printRef} className="p-6">
          <div className="voucher" style={{ border: "2px solid #000", padding: "20px", maxWidth: "700px", margin: "0 auto", fontFamily: "'Traditional Arabic', Arial, sans-serif", direction: "rtl", color: "#000" }}>
            {/* Header */}
            <div style={{ textAlign: "center", marginBottom: "15px", borderBottom: "2px solid #000", paddingBottom: "10px" }}>
              ...
              <h1 style={{ fontSize: "22px", fontWeight: "bold", margin: "8px 0" }}>{TRANSACTION_TYPE_LABELS[tx.type]}</h1>
              <p style={{ fontSize: "14px" }}>({tx.type === "receipt" ? "المادة" : tx.type === "payment" ? "المادة" : "المادة"})</p>
            </div>
            ...
            {isReceipt && (
              <>
                <table style={{ width: "100%", borderCollapse: "collapse", margin: "15px 0" }}>
                  ...
                  <tbody>
                    <tr>
                      <td style={{ border: "1px solid #000", padding: "8px", textAlign: "right", minHeight: "60px" }}>
                        {tx.description}
                        <br /><br />
            ...
            {isPayment && (
              <>
                <p style={{ fontSize: "14px", marginBottom: "10px" }}>
                  ...
                  <span style={{ borderBottom: "1px dotted #000", display: "inline-block", minWidth: "200px" }}>
                    {accounts.credits.map(c => c.label).join("، ")}
                  </span>
                </p>
                <table style={{ width: "100%", borderCollapse: "collapse", margin: "15px 0" }}>
                  <thead>
                    <tr>
                      <th style={{ border: "1px solid #000", padding: "8px", background: "#f0f0f0", textAlign: "right", width: "60%" }}>تفاصـــــــــــــيل</th>
                      <th style={{ border: "1px solid #000", padding: "8px", background: "#f0f0f0" }} colSpan={2}>المبلغ</th>
                    </tr>
                    <tr>
                      <th style={{ border: "1px solid #000", padding: "6px", background: "#f5f5f5" }}></th>
                      <th style={{ border: "1px solid #000", padding: "6px", background: "#f5f5f5", fontSize: "12px" }}>دينار</th>
                  ...
                  <tbody>
                    {accounts.credits.map((c, i) => {
                      ...
                      return (
                        <tr key={i}>
                          <td style={{ border: "1px solid #000", padding: "8px", textAlign: "right" }}>{c.label} - {tx.description}</td>
                          <td style={{ border: "1px solid #000", padding: "8px", textAlign: "center" }}>{s.dinars}</td>
                          <td style={{ border: "1px solid #000", padding: "8px", textAlign: "center" }}>{s.fils}</td>
                        </tr>
                      );
                    })}
                    <tr style={{ background: "#e8e8e8", fontWeight: "bold" }}>
                      <td style={{ border: "1px solid #000", padding: "8px", textAlign: "right" }}>المجموع</td>
                      <td style={{ border: "1px solid #000", padding: "8px", textAlign: "center" }}>{splitAmount(totals.credit).dinars}</td>
                      <td style={{ border: "1px solid #000", padding: "8px", textAlign: "center" }}>{splitAmount(totals.credit).fils}</td>
                    </tr>
                  </tbody>
                ...
                <p style={{ fontSize: "13px", margin: "10px 0" }}>
                  أصادق على صحة البيان المذكور أعلاه وأشهد أن الاتفاق قد تم وفقاً للنظام وتعليماته.
                </p>
              </>
            )}
            ...
            {/* Payment method for payment vouchers */}
            {isPayment && (
              <div style={{ marginTop: "20px", fontSize: "13px", borderTop: "1px solid #ccc", paddingTop: "10px" }}>
                <p>رقم التحويل (الشيك): ........................  تاريخه: ....../....../......</p>
                <p style={{ marginTop: "5px" }}>الدفع نقداً □</p>
              </div>
            )}
          </div>
        </div>
