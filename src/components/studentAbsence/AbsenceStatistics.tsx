...
import { CalendarIcon, FileText, FileDown, BarChart3, Users, School, GraduationCap } from "lucide-react";
...
type ViewLevel = "school" | "grade" | "section";
...
export default function AbsenceStatistics({ userId, schoolName, directorateName, principalName }: Props) {
  ...
  // Extract unique grades and sections from students
  ...
  const sections = useMemo(() => {
    ...
    const set = new Set<string>();
    students.forEach(s => {
      if (s.className.startsWith(filterGrade)) {
        set.add(s.className);
      }
    });
    return [...set].sort();
  ...
  const getStudentsByLevel = useMemo(() => {
    ...
    if (viewLevel === "grade" && filterGrade) {
      return students.filter(s => s.className.startsWith(filterGrade));
    }
    if (viewLevel === "section" && filterSection) {
      return students.filter(s => s.className === filterSection);
    }
    return students;
  }, [students, viewLevel, filterGrade, filterSection]);

  // Get absence records by level
  const levelFilteredRecords = useMemo(() => {
    if (viewLevel === "school") return dateFilteredRecords;
    if (viewLevel === "grade" && filterGrade) {
      return dateFilteredRecords.filter(r => r.className.startsWith(filterGrade));
    }
    if (viewLevel === "section" && filterSection) {
      return dateFilteredRecords.filter(r => r.className === filterSection);
    }
  ...
  // Breakdown by class for school/grade view
  ...
  // Per-student detail for section view
  ...
  return (
    <div className="space-y-4">
      ...
      {classBreakdown.length > 0 && viewLevel !== "section" && (
        <Card>
          <CardHeader>
